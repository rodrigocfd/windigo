...
func (me *WindowMain) RunAsMain() {
	...
	cxScreen := api.GetSystemMetrics(api.SM_CXSCREEN) // retrieve screen size
	cyScreen := api.GetSystemMetrics(api.SM_CYSCREEN)
...
type windowMainSetup struct {
	...
	CmdShow api.SW // Passed to ShowWindow, defaults to SW_SHOW.
}
...
func (me *windowMainSetup) initOnce() {
	if !me.wasInit {
		...
		me.Height = 500
		me.Style = api.WS_CAPTION | api.WS_SYSMENU | api.WS_CLIPCHILDREN | api.WS_BORDER
		me.ExStyle = api.WS_EX(0)

		me.CmdShow = api.SW_SHOW
	}
}
