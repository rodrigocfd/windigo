...
type IBindCtx struct{ win.IUnknown }

// Constructs a COM object from the base IUnknown.
//
// ⚠️ You must defer IBindCtx.Release().
func NewIBindCtx(base win.IUnknown) IBindCtx {
...
func (me *IBindCtx) ReleaseBoundObjects() {
	ret, _, _ := syscall.Syscall(
		(*shellvt.IBindCtx)(unsafe.Pointer(*me.Ptr())).ReleaseBoundObjects, 1,
...
func (me *IBindCtx) RevokeObjectParam(key string) {
	ret, _, _ := syscall.Syscall(
		(*shellvt.IBindCtx)(unsafe.Pointer(*me.Ptr())).RevokeObjectParam, 2,
		uintptr(unsafe.Pointer(me.Ptr())),
