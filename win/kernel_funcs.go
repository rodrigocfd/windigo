...
func AllocConsole() error {
	ret, _, err := syscall.SyscallN(
		dll.Kernel.Load(&_kernel_AllocConsole, "AllocConsole"))
	return utl.ZeroAsGetLastError(ret, err)
}
...
//
// [GetCurrentPackageFamilyName]: https://learn.microsoft.com/en-us/windows/win32/api/appmodel/nf-appmodel-getcurrentpackagefamilyname
func GetCurrentPackageFamilyName() (string, error) {
	var szBuf uint32
	ret, _, _ := syscall.SyscallN(
		dll.Kernel.Load(&_kernel_GetCurrentPackageFamilyName, "GetCurrentPackageFamilyName"),
...
var _kernel_GetCurrentPackageFamilyName *syscall.Proc
...
//
// [GetCurrentPackagePath]: https://learn.microsoft.com/en-us/windows/win32/api/appmodel/nf-appmodel-getcurrentpackagepath
func GetCurrentPackagePath() (string, error) {
	var szBuf uint32
	ret, _, _ := syscall.SyscallN(
		dll.Kernel.Load(&_kernel_GetCurrentPackagePath, "GetCurrentPackagePath"),
		uintptr(unsafe.Pointer(&szBuf)),
		0)
	if wErr := co.ERROR(ret); wErr != co.ERROR_INSUFFICIENT_BUFFER {
		return "", wErr
