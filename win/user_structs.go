...
type ICONINFOEX struct {
	...
	HbmColor  HBITMAP
	WResID    uint16
	szModName [util.MAX_PATH]uint16
	szResName [util.MAX_PATH]uint16
}
...
func (iix *ICONINFOEX) SetFIcon(val bool) {
	iix.fIcon = util.BoolToInt32(val)
}
...
func (ps *PAINTSTRUCT) SetFErase(val bool) {
	ps.fErase = util.BoolToInt32(val)
}
...
type TITLEBARINFOEX struct {
	cbSize     uint32
	RcTitleBar RECT
	Rgstate    [util.CCHILDREN_TITLEBAR + 1]uint32
	Rgrect     [util.CCHILDREN_TITLEBAR + 1]RECT
}
