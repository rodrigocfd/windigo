...
type DXGI_ADAPTER_DESC1 struct {
	...
	SharedSystemMemory    uint
	AdapterLuid           co.LUID
	Flags                 codxgi.DXGI_ADAPTER_FLAG
}
...
type DXGI_GAMMA_CONTROL_CAPABILITIES struct {
	ScaleAndOffsetSupported win.BOOL
...
type DXGI_MODE_DESC struct {
	...
	Format           codxgi.DXGI_FORMAT
	ScanlineOrdering codxgi.DXGI_MODE_SCANLINE_ORDER
	Scaling          codxgi.DXGI_MODE_SCALING
}
...
type DXGI_OUTPUT_DESC struct {
	...
	AttachedToDesktop  win.BOOL
	Rotation           codxgi.DXGI_MODE_ROTATION
...
type DXGI_PRESENT_PARAMETERS struct {
	...
	pDirtyRects     *win.RECT
	PScrollRect     *win.RECT
	PScrollOffset   *win.POINT
}

// Returns a slice over the pDirtyRects field.
func (pp *DXGI_PRESENT_PARAMETERS) DirtyRects() []win.RECT {
	return unsafe.Slice(pp.pDirtyRects, pp.dirtyRectsCount)
}

// Sets the pDirtyRects and dirtyRectsCount fields.
func (pp *DXGI_PRESENT_PARAMETERS) SetDirtyRects(val []win.RECT) {
...
type DXGI_SWAP_CHAIN_DESC struct {
	...
	SampleDesc   DXGI_SAMPLE_DESC
	BufferUsage  codxgi.DXGI_USAGE
	BufferCount  uint32
	OutputWindow win.HWND
	Windowed     win.BOOL
	SwapEffect   codxgi.DXGI_SWAP_EFFECT
...
type DXGI_SWAP_CHAIN_DESC1 struct {
	...
	BufferUsage codxgi.DXGI_USAGE
	BUfferCount uint32
	Scaling     codxgi.DXGI_SCALING
	SwapEffect  codxgi.DXGI_SWAP_EFFECT
	AlphaMode   codxgi.DXGI_ALPHA_MODE
	Flags       codxgi.DXGI_SWAP_CHAIN_FLAG
}
