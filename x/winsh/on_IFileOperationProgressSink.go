...
//		func(
//			flags cosh.TSF,
//			item, destFolder *winsh.IShellItem,
//			newName string,
//		) co.HRESULT {
...
type IFileOperationProgressSink struct{ win.IUnknown }
...
type _IFileOperationProgressSinkImpl struct {
	...
	preRenameItem    func(flags cosh.TSF, item *IShellItem, newName string) co.HRESULT
	postRenameItem   func(flags cosh.TSF, item *IShellItem, newName string, hrRename co.HRESULT, newlyCreated *IShellItem) co.HRESULT
	preMoveItem      func(flags cosh.TSF, item, destFolder *IShellItem, newName string) co.HRESULT
	postMoveItem     func(flags cosh.TSF, item, destFolder *IShellItem, newName string, hrMove co.HRESULT, newlyCreated *IShellItem) co.HRESULT
	preCopyItem      func(flags cosh.TSF, item, destFolder *IShellItem, newName string) co.HRESULT
	postCopyItem     func(flags cosh.TSF, item, destFolder *IShellItem, newName string, hrMove co.HRESULT, newlyCreated *IShellItem) co.HRESULT
	preDeleteItem    func(flags cosh.TSF, item *IShellItem) co.HRESULT
	postDeleteItem   func(flags cosh.TSF, item *IShellItem, hrDelete co.HRESULT, newlyCreated *IShellItem) co.HRESULT
	preNewItem       func(flags cosh.TSF, destFolder *IShellItem, newName string) co.HRESULT
	postNewItem      func(flags cosh.TSF, destFolder *IShellItem, newName, templateName string, attr co.FILE_ATTRIBUTE, hrNew co.HRESULT, newItem *IShellItem) co.HRESULT
...
//
// [IFileOperationProgressSink]: https://learn.microsoft.com/en-us/windows/win32/api/shobjidl_core/nn-shobjidl_core-ifileoperationprogresssink
func NewIFileOperationProgressSinkImpl(releaser *win.OleReleaser) *IFileOperationProgressSink {
	...
	return utl.OleNew[*IFileOperationProgressSink](uintptr(unsafe.Pointer(ppImpl)), releaser)
}
	...
	*me = _IFileOperationProgressSinkVt{
		...
		PreRenameItem: syscall.NewCallback(
			func(
				...
				if fun := (**ppImpl).preRenameItem; fun == nil { // user didn't define a callback
					...
					return uintptr(fun(
						dwFlags,
						utl.OleNewWithoutReleaser[*IShellItem](psiItem),
			...
			func(
				...
				if fun := (**ppImpl).postRenameItem; fun == nil { // user didn't define a callback
					...
					return uintptr(fun(
						...
						wstr.DecodePtr(pszNewName),
						hrRename,
						utl.OleNewWithoutReleaser[*IShellItem](psiNewlyCreated),
					))
				}
			},
		),
		PreMoveItem: syscall.NewCallback(
			func(
				ppImpl **_IFileOperationProgressSinkImpl,
				dwFlags cosh.TSF,
				psiItem, psiDestinationFolder uintptr,
				pszNewName *uint16,
			) uintptr {
				if fun := (**ppImpl).preMoveItem; fun == nil { // user didn't define a callback
					...
					return uintptr(fun(
						dwFlags,
						utl.OleNewWithoutReleaser[*IShellItem](psiItem),
						utl.OleNewWithoutReleaser[*IShellItem](psiDestinationFolder),
						wstr.DecodePtr(pszNewName),
					))
				}
			},
		...
		PreDeleteItem: syscall.NewCallback(
			func(ppImpl **_IFileOperationProgressSinkImpl, dwFlags cosh.TSF, psiItem uintptr) uintptr {
			...
			func(
				...
				if fun := (**ppImpl).postNewItem; fun == nil { // user didn't define a callback
					...
					return uintptr(fun(
						...
						fileAttributes,
						hrNew,
						utl.OleNewWithoutReleaser[*IShellItem](psiNewItem),
					))
				}
			},
		),
